			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="add-form" class="pl-3 pr-3">
						<?= csrf_field(); ?>
						<div class="row">
							<input type="hidden" id="id" name="id" class="form-control" placeholder="Id" maxlength="5" required>
						</div>
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									<label for="namaKategori"> Nama kategori: <span class="text-danger">*</span> </label>
									<input type="text" id="namaKategori" name="namaKategori" class="form-control" placeholder="Nama kategori" maxlength="50" required>
								</div>
							</div>
						</div>
...
<script>
	$(function() {
		$('#data_table').DataTable({
			...
			"responsive": true,
			"ajax": {
				"url": '<?php echo base_url('kategoriBarang/getAll') ?>',
				"type": "POST",
				"dataType": "json",
				async: "true"
			}
	...
	function add() {
		...
		$.validator.setDefaults({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: '<?php echo base_url('kategoriBarang/add') ?>',
					type: 'post',
	...
	function edit(id) {
		$.ajax({
			url: '<?php echo base_url('kategoriBarang/getOne') ?>',
			type: 'post',
			data: {
				id: id
			},
			...
			success: function(response) {
				...
				$("#edit-form #namaKategori").val(response.nama_kategori);
				...
				$.validator.setDefaults({
					...
					submitHandler: function(form) {
						...
						$(".text-danger").remove();
						$.ajax({
							url: '<?php echo base_url('kategoriBarang/edit') ?>',
							type: 'post',
							data: form.serialize(),
							dataType: 'json',
							beforeSend: function() {
	...
	function remove(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: '<?php echo base_url('kategoriBarang/remove') ?>',
					type: 'post',
					data: {
						id: id
					},
